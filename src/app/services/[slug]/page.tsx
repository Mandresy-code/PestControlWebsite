    ...
    <>
      ...
      <div className="bg-navy-900 pt-[112px] pb-72">
        <div className="container-site">
          <Link
            href="/services"
            className="inline-flex items-center gap-8 text-navy-300 hover:text-white text-sm font-medium mb-32 transition-colors duration-micro focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/55 rounded-sm"
          >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-8 mb-24">
                {service.chips.map((chip) => (
                  <Chip key={chip} onDark>{chip}</Chip>
                ))}
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-[1fr_320px] gap-72">
            ...
            <div>
                ...
                {service.methods.map((method, i) => (
                  <li key={i} className="flex items-start gap-16">
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-navy-900/8 shrink-0 mt-0.5">
                      <span className="font-mono text-[10px] text-navy-500 tabular-nums">
              ...
              <div className="rounded-lg bg-navy-900 p-32 text-white">
                <p className="font-mono text-eyebrow uppercase tracking-widest text-signal-400 mb-12">
                  Approche IPM
                </p>
                <p className="text-body text-navy-200 leading-relaxed">
                  ...
                  diagnostic d&apos;abord, méthode mécanique ou physique si possible, biocide
                  en dernier recours — toujours avec un technicien Certibiocide.
                </p>
              </div>
            </div>
            ...
            <aside className="flex flex-col gap-24">
              <div className="bg-paper rounded-lg border border-navy-900/8 p-24 shadow-1">
                ...
                <ul className="flex flex-col gap-12">
                  {[
                    "Diagnostic terrain avant intervention",
              ...
              {service.urgent && (
                <div className="bg-danger/8 border border-danger/20 rounded-lg p-20">
                  ...
                  <p className="text-sm text-navy-600">
                    Ce nuisible nécessite une prise en charge rapide. Nous traitons votre
                    demande en priorité.
                  </p>
                </div>
              )}
          ...
          <div className="grid sm:grid-cols-3 gap-20">
            {related.map((s) => (
              <Link
                ...
                className="group flex flex-col gap-12 p-24 bg-paper rounded-lg border border-navy-900/8 hover:shadow-2 transition-shadow duration-standard ease-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/55"
              >
                <p className="font-mono text-mono text-navy-400">{s.index}</p>
                <h3 className="text-body font-medium text-navy-900 group-hover:text-navy-700 transition-colors duration-micro">
                  {s.title}
                </h3>
              </Link>
            ))}
