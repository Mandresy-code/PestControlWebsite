...
import { pests, wizardPlaces } from "@/lib/content";
...
// ─── Inner wizard (reads searchParams) ─────────────────────────────────────────
...
function WizardInner() {
  ...
  useEffect(() => {
    ...
    if (pestParam) {
      dispatch({ type: "PREFILL_PEST", pest: pestParam });
    }
  }, [searchParams]);
  ...
  const canNext1 = !!state.place;
  const canNext2 = !!state.pest;
  const canNext3 = !!state.situation;
  ...
  return (
    <div>
          ...
          >
            <div
              className={cn(
                ...
                s < state.step  ? "bg-navy-900" :
                s === state.step ? "bg-signal-500" :
                "bg-navy-900/15"
              )}
            />
            <span className={cn(
              "font-mono text-[10px] uppercase tracking-widest",
              s === state.step ? "text-navy-900" : "text-navy-400"
            )}>
              {stepLabels[s]}
            </span>
          </div>
      ...
      {/* Step 1 — Lieu */}
      {state.step === 1 && (
        <StepCard title="Quel type de lieu ?" step={1}>
          <div className="grid sm:grid-cols-2 gap-12">
            {wizardPlaces.map(({ id, label, icon }) => (
              <ChoiceButton
                key={id}
                label={label}
                selected={state.place === id}
                onClick={() => dispatch({ type: "SET_PLACE", payload: id })}
              />
            ))}
          </div>
          <NavRow onNext={() => dispatch({ type: "NEXT" })} nextDisabled={!canNext1} />
        </StepCard>
      )}

      {/* Step 2 — Nuisible */}
      {state.step === 2 && (
        <StepCard title="Quel nuisible pensez-vous avoir ?" step={2}>
          <div className="grid sm:grid-cols-2 gap-12">
            {pests.map((pest) => (
              <ChoiceButton
                key={pest.id}
      ...
      {/* Step 3 — Situation */}
      {state.step === 3 && (
        <StepCard title="Comment décririez-vous la situation ?" step={3}>
      ...
      {/* Step 4 — Coordonnées */}
      {state.step === 4 && (
        <StepCard title="Vos coordonnées" step={4}>
          ...
          {state.errors.submit && (
            <p className="text-sm text-danger mt-12" role="alert">{state.errors.submit}</p>
          )}
          <NavRow
            onPrev={() => dispatch({ type: "PREV" })}
            onNext={() => dispatch({ type: "NEXT" })}
            nextDisabled={!state.nom || !state.email}
          />
        </StepCard>
      )}
      ...
      {/* Step 5 — Confirmation */}
      {state.step === 5 && (
        <StepCard title="Récapitulatif" step={5}>
          ...
          <div className={cn(
            ...
            (state.pest === "rats" || state.pest === "punaises" || state.pest === "guepes")
...
function StepCard({ title, children }: { title: string; step: Step; children: React.ReactNode }) {
  return (
    <div>
...
function ChoiceButton({
  label, sub, selected, onClick, badge,
}: {
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : "border-navy-900/12 hover:border-signal-500 hover:bg-signal-500/5"
      )}
    >
      <div>
        <p className={cn("text-body font-medium", selected ? "text-navy-900" : "text-navy-800")}>
          {label}
        </p>
      ...
      <div className="flex items-center gap-10 shrink-0">
        ...
        <div
          className={cn(
            "w-5 h-5 rounded-full border-2 transition-colors duration-micro",
...
function Confirmation({ state }: { state: FormState }) {
  const urgent = state.pest === "rats" || state.pest === "punaises" || state.pest === "guepes";
  return (
